...
async function main() {
  ...
  const { uuid, version: version2, valid: initiallyValid } = JSON.parse(uploadResponse);
  core.info(
    `Successfully uploaded add-on for version ${version2} with uuid ${uuid}`
  );
  ...
  try {
    ...
    while (!valid) {
      ...
      const { valid: valid2, validation } = JSON.parse(
        await getFromAmo(`/api/v5/addons/upload/${uuid}`)
      );
      if (valid2) {
        core.info("Upload is valid");
        break;
      } else if (valid2 === false && validation) {
...
async function postToAmo({ path: path2, jsonData }) {
  const url = `https://${AMO_HOST}${path2}`;
  const res = await fetch(url, {
    method: "POST",
    headers: {
      ...
      "Content-Length": String(import_utf8.default.encode(jsonData).length)
    },
    body: jsonData
  });
  return res.text();
}
function uploadToAmo({ path: path2, formData, method = "POST" }) {
  core.info(`Uploading to https://${AMO_HOST}${path2} (${method})`);
  return new Promise((resolve, reject) => {
    formData.submit(
      {
        host: AMO_HOST,
      ...
      (err, res) => {
        ...
        if (!res.statusCode || res.statusCode < 200 || res.statusCode >= 300) {
          reject(
            new Error(
              `Got status ${res.statusCode} for POST https://${AMO_HOST}${path2}`
            )
          );
        } else {
          let body = "";
